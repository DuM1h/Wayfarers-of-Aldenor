...
public class TurnManager
{
    ...
    public void TickFreeTurn()
    {
        ...
        {
            if (character != _playerCharacter && !character.Stats.IsDead)
            {
    ...
    public void CheckForCombatTriggers()
    {
        foreach (var character in _allCharacters)
        {
            if (!character.Stats.IsDead && character is EnemyBrain enemy)
            {
    ...
    public void ReleaseNode(Vector2Int position)
    {
        Node node = _gameGrid.GetNode(position);
        if (node != null)
        {
